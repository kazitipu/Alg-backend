...
class RechargeWalletModal extends Component {
  ...
  componentWillReceiveProps = (nextProps) => {
    const { rechargeRequestObj } = nextProps;
    if (rechargeRequestObj) {
      this.setState({
        paymentMethod: rechargeRequestObj.method.includes("Bank")
          ? "Bank"
          : "Mobile Banking",
        mobileBanking: !rechargeRequestObj.method.includes("Bank")
          ? rechargeRequestObj.method
          : "",
        bankName: rechargeRequestObj.method.includes("Bank")
          ? rechargeRequestObj.method
          : "",
        amount: rechargeRequestObj.amount,
      });
    }
  };
  ...
  handleSubmit = async (event) => {
    ...
    const { userObj, admin, rechargeRequestObj, rechargeUser } = this.props;
    ...
    if (rechargeRequestObj) {
      ...
      await this.props.rechargeUserFromRechargeRequestRedux({
        ...this.state,
        uid: rechargeRequestObj.userId,
        rechargedAt,
        day,
        rechargeId: rechargeRequestObj.rechargeId,
        rechargeBy: admin ? admin.name : "",
        status: "recharged",
      });
      toast.success(
        `Recharge succesful to ${rechargeUser.displayName} of amount ${this.state.amount}`
      );
    }
  ...
  render() {
    const { rechargeRequestObj, rechargeUser, userObj } = this.props;
    console.log(rechargeRequestObj);
                      ...
                      <div className="d-lg-flex justify-content-end no-gutters mb-spacer-md">
                        <div className="col">
                          <div className="px-3 m-5">
                            ...
                            >
                              {rechargeUser
                                ? `Recharge ${rechargeUser.displayName}`
                                : "Recharge"}
                            </h2>
                            <form
                              onSubmit={this.handleSubmit}
...
const mapStateToProps = (state, ownProps) => {
  console.log(ownProps.rechargeRequestObj);
  return {
    allUsers: state.users.users,
    rechargeUser: ownProps.rechargeRequestObj
      ? state.users.users.find(
          (user) => user.uid === ownProps.rechargeRequestObj.userId
        )
      : null,
